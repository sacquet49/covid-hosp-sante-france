...
export class HospAgeComponent implements AfterViewInit, OnInit {
  ...
  init(): void {
    if (this.hospService.csv[7].data.length > 0 && this.hospitaliseParJour.length === 0) {
      this.hospitaliseParJour = this.hospService.csv[7].data.reduce((r, v, i, a, k = v.jour) => ((r[k] || (r[k] = [])).push(v), r), {});
